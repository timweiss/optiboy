...
export const updateEntry = async (entry: EmailEntry) => {
  await pool.query('UPDATE entries SET confirmed = ($1), confirmation_key = ($2), updated_at = ($3) WHERE id = ($4)', [entry.confirmed, entry.confirmationKey, entry.updatedAt.toISOString(), entry.id]);
};
